...
def test_web_app(page: Page):
    ...
    logger.log(INFO, "Step 1.1: Manage popup(s)")
    popup_button = page.locator('//button[contains(@class,"ScCoreButton")]/div[contains(@class, "ScCoreButtonLabel")]').last
    try:
        # page.expect_popup()
        expect(popup_button).to_be_visible()
        print("Cookies popup is visible")
        popup_button.click()
        print("Cookies popup clicked to close")
